...
void read_button_and_firebase() {
  if (start_st) {
    Firebase.getInt(fbdb, "/tu_ap_2/set_val/temp_max");
    if (set_temp_max != fbdb.intData()) {
      set_temp_max = fbdb.intData();
    }
    Firebase.getInt(fbdb, "/tu_ap_2/set_val/temp_min");
    if (set_temp_min != fbdb.intData()) {
      set_temp_min = fbdb.intData();
    }
    Firebase.getInt(fbdb, "/tu_ap_2/set_val/humi_max");
    if (set_humi_max != fbdb.intData()) {
      set_humi_max = fbdb.intData();
    }
    Firebase.getInt(fbdb, "/tu_ap_2/set_val/humi_min");
  ...
  if (btstart) {
    ...
    Firebase.setInt(fbdb, "/tu_ap_2/start", start_st);
  } else {
    Firebase.getInt(fbdb, "/tu_ap_2/start");
  ...
  if (btmode) {
    ...
    Firebase.setInt(fbdb, "/tu_ap_2/mode", mode_st);
  } else {
    Firebase.getInt(fbdb, "/tu_ap_2/mode");
  ...
  if (btfan) {
    ...
    if (!mode_st) {
      ...
      Firebase.setInt(fbdb, "/tu_ap_2/fan", fan_st);
    }
  } else {
    if (!mode_st) {
      Firebase.getInt(fbdb, "/tu_ap_2/fan");
...
void send_firebase() {
  Firebase.setInt(fbdb, "/tu_ap_2/temp", temp_val);
  Firebase.setInt(fbdb, "/tu_ap_2/humi", humi_val);
  digitalWrite(LED_light, light_st);
  digitalWrite(LED_mist, mist_st);
  digitalWrite(LED_fan, fan_st);
  digitalWrite(LED_canh_bao, canh_bao_led);
  Firebase.setInt(fbdb, "/tu_ap_2/light", light_st);
  Firebase.setInt(fbdb, "/tu_ap_2/mist", mist_st);
  Firebase.setInt(fbdb, "/tu_ap_2/fan", fan_st);
  Firebase.setInt(fbdb, "/tu_ap_2/connect", connect);
  Firebase.setInt(fbdb, "/tu_ap_2/warning", canh_bao_led);
}
